...
main() {
    ...
    case "$DISTRO" in
        ...
        Fedora)
            mapfile -t packages < <(
                "${LIST_CMD[@]}" "${pkg_names[@]}" 2>&1 |
                awk '!/(^$|^=|---|Dependencies resolved|Transaction Summary|Is this ok|Last metadata|Running transaction|Total download size|^Package |^Arch |^Version |^Repository |^Size |Installing:|Updating:)/ {print $1}'
            )
            total=${#packages[@]}
            ;;
        OpenSuse)
