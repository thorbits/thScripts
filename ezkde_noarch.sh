...
KDE_GROUP[opensuse]="plasma6-desktop plasma6-workspace kwwin6 sddm konsole discover pipewire" #"patterns-kde-kde_plasma konsole discover dolphin pipewire sddm"
    ...
    case "$DISTRO" in
        ...
        opensuse)
            mapfile -t packages < <(
				...
				awk '/new/ {for(i=1;i<=NF;i++) if ($i ~ /^[a-zA-Z0-9.-]+$/) print $i}' | head -n -5 | grep -v "session-x11" | grep -v "kwin6-x11"
			)
